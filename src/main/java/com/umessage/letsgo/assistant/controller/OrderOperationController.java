...
public class OrderOperationController {
    ...
    public Map<String,Object> updateStatus(HttpServletRequest request,Long id) throws BusinessException {
        ...
        try {
            ...
            //为推荐人增加收益
            //修改微信信息表的收益
            logger.info("父的openid 为："+ wechatOrderInfoEntity.getSellerOpenid());
            if(null!= wechatOrderInfoEntity.getSellerOpenid()){
                logger.info("修改收益");
                WeChatInfoEntity weChatInfoEntity = weChatInfoService.findWecahtUserByOpenID(wechatOrderInfoEntity.getSellerOpenid());
                double reward = null==weChatInfoEntity.getReward()?0.0:weChatInfoEntity.getReward();
                logger.info("收益为：" + reward);
                weChatInfoEntity.setReward(reward + (wechatOrderInfoEntity.getTotalFee() - wechatOrderInfoEntity.getRealFee()));
                weChatInfoService.updateWechatInf(weChatInfoEntity);
            }
            //TODO 推送订单完成模板消息
